export async function onRequestPost({ request, env }) {
  try {
    ...
    const { subject, heading, body_text, cta_text, cta_url, segment, region_filter, member_ids, test_email } = body;
    ...
    if (test_email) {
      recipients = [{ email: test_email, full_name: 'Admin' }];
    } else if (member_ids && member_ids.length > 0) {
      const idList = member_ids.join(',');
      const res = await fetch(
        `${env.SUPABASE_URL}/rest/v1/members?select=id,email,full_name&id=in.(${idList})`,
        { headers: supabaseHeaders }
      );
      const data = await res.json();
      recipients = Array.isArray(data) ? data.filter(m => m.email) : [];
    } else {
      let query = `${env.SUPABASE_URL}/rest/v1/members?select=id,email,full_name`;
      if (segment === 'active') query += '&status=eq.active';
      else if (segment === 'active_pending') query += '&status=in.(active,pending)';
      if (region_filter) query += `&region=eq.${encodeURIComponent(region_filter)}`;
    ...
    if (recipients.length === 0) {
      return new Response(JSON.stringify({ error: 'No hay destinatarios con email para ese segmento.' }), { status: 400 });
    }
  ...
  <div style="background:#f8fafc;padding:30px 20px;text-align:center;border-bottom:1px solid #e2e8f0">
    <img src="https://club-hogga.pages.dev/images/logo.webp" alt="Club Hogga" style="height:48px">
  </div>
  ...
  <div style="background:#f8fafc;padding:20px;text-align:center;border-top:1px solid #e2e8f0">
    <p style="font-size:13px;color:#64748b;margin:0">Si tienes alguna duda, escríbenos a club@hogga.cl.</p>
    ...
    for (let i = 0; i < recipients.length; i += BATCH) {
      ...
      const emails = batch.map(r => ({
        ...
        to: [r.email],
        subject,
        html: buildHtml(r.full_name ? r.full_name.split(' ')[0] : '')
      }));
